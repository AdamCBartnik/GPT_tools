...
from .postprocess import clip_to_charge, clip_to_emit
from .cathode_particlegroup import get_cathode_particlegroup
...
def run_gpt_with_settings(settings=None,
    ...
    if ('merit:z' in settings.keys()):
        ...
        if (used_filter):
            ...
            n_tout = np.count_nonzero(G_all.stat('mean_z', 'tout') <= z + 1.0e-9) # slightly larger that z to avoid floating point comparison problem
            n_screen = np.count_nonzero(G_all.stat('mean_z', 'screen') <= z + 1.0e-9) # slightly larger that z to avoid floating point comparison problem
...
def keep_only_last_forward_pass(PG_input):
    if (len(PG_input) == 0):
        return PG_input
    # For each id keep the entry with the latest time (first one on ties), in order of each id's first appearance
    ids = PG_input.id
    order = np.lexsort((-PG_input.t, ids))                   # by id, then time descending (stable)
    sorted_ids = ids[order]
    first_of_group = np.r_[True, sorted_ids[1:] != sorted_ids[:-1]]
    latest_index = order[first_of_group]                     # one per id, ids in ascending order
    _, first_appearance = np.unique(ids, return_index=True)  # also ids in ascending order
    indices_to_keep = latest_index[np.argsort(first_appearance)]
...
def index_of_ids(ids, sorted_ids, id_order):
    # Positions in the original particle group of each id; searchsorted alone silently maps unknown ids to a neighbor
    pos = np.clip(np.searchsorted(sorted_ids, ids), 0, len(sorted_ids)-1)
...
def multithread_gpt_with_settings(settings=None,
    ...
    if (n_threads is None):
        ...
        if (verbose):
            print(f'Using maximum of {n_threads} threads...')
            ...
            for s in g.screen:
                if (len(s) > 0):
                    ...
                    if (np.abs(z_list[which_screen] - z) < ztol):
                        ...
                        p_ii = index_of_ids(int_id, sorted_ids, id_order)
